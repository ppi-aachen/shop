...
async function getProductsFromGoogleSheet(): Promise<CartItem[]> {
  try {
...
async function updateStockInGoogleSheet(itemId: number, newStock: number) {
  try {
    const accessToken = await getGoogleSheetsAuth()
    const response = await fetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${GOOGLE_SHEET_ID}/values/Products!M${itemId + 1}?valueInputOption=USER_ENTERED`,
      {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${accessToken}`,
...
async function sendCustomerConfirmationEmail(orderData: OrderData, orderItems: OrderItemData[]) {
  ...
  `
    ...
    const { data, error } = await emailResponse
...
async function sendBusinessNotificationEmail(orderData: OrderData, orderItems: OrderItemData[]) {
  ...
  `
    ...
    const emailResponse = resend.emails.send({
      from: "Aachen Studio <funding@ppiaachen.de>",
