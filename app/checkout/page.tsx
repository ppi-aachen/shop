          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              <div className="space-y-4 mb-6">
                {useCart().state.items.map((item, index) => (
                  <div
              ...
              <div className="border-t pt-4 mb-4">
                <div className="flex items-center gap-2 mb-2">
                  {useCart().state.deliveryMethod === "pickup" ? (
                    <MapPin className="h-4 w-4 text-green-600" />
                  ) : (
                    <Truck className="h-4 w-4 text-green-600" />
                  )}
                  <span className="font-medium">
                    {useCart().state.deliveryMethod === "pickup" ? "Pickup in Aachen" : "Delivery"}
                  </span>
                </div>
                {useCart().state.deliveryMethod === "pickup" && (
              ...
              <div className="border-t pt-4">
                <div className="space-y-2">
                  <div className="flex justify-between">
                    <span>Items ({useCart().state.itemCount})</span>
                    <span>€{useCart().state.total.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>{useCart().state.deliveryMethod === "pickup" ? "Pickup" : "Delivery"}</span>
                    <span>€{useCart().state.shippingCost.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between items-center text-xl font-bold border-t pt-2">
                    <span>Total</span>
                    <span className="text-green-600">€{useCart().state.finalTotal.toFixed(2)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-6 p-4 bg-blue-50 rounded-lg">
                ...
                <p className="text-blue-800 text-sm">
                  Please transfer €{useCart().state.finalTotal.toFixed(2)} to our account and upload the proof of
                  payment:
                </p>
