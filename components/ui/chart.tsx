...
>(({ className, type, data, x, y, label, color, ...props }, ref) => {
  const ChartComponent =
    type === "bar" ? VictoryBar : type === "line" ? VictoryLine : type === "pie" ? VictoryPie : null
  ...
  return (
    <div ref={ref} className={cn("w-full h-64", className)} {...props}>
      <VictoryChart theme={VictoryTheme.material} domainPadding={type === "bar" ? 20 : 0}>
        ...
        <ChartComponent
          ...
          x={x}
          y={y}
          labels={label ? ({ datum }) => datum[label] : undefined}
          style={{ data: { fill: color || "steelblue" } }}
        />
      </VictoryChart>
    </div>
  )
